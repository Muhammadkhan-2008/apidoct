...
export function SettingsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup maxWidth="max-w-3xl" className="flex flex-col overflow-y-hidden p-0 glass-card rounded-3xl border border-white/10 shadow-2xl bg-card/90 backdrop-blur-2xl">
        <div className="flex items-center justify-between gap-4 border-b border-border/40 px-6 py-4 bg-accent/10">
          <DialogTitle className="text-base font-extrabold text-foreground flex items-center gap-2">
            <SlidersHorizontal className="size-4 text-indigo-400" />
            ApiDoct Gateway Control Center
          </DialogTitle>
          <DialogClose
            aria-label={t('common.dismiss')}
            className="-me-1 rounded-xl p-1.5 text-muted-foreground transition-colors outline-none hover:text-foreground hover:bg-accent/40"
          >
            <X className="size-4" />
          </DialogClose>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
          <nav
            ...
            className="flex shrink-0 gap-1 overflow-x-auto border-b border-border/40 p-2 sm:w-52 sm:flex-col sm:overflow-x-visible sm:border-b-0 sm:border-e sm:p-4 bg-card/40"
          >
            {SECTIONS.map(item => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex shrink-0 items-center gap-2.5 rounded-2xl px-3.5 py-2.5 text-start text-xs font-bold transition-all duration-200 outline-none ${
                    selected
                      ? 'bg-gradient-to-r from-indigo-600/30 via-purple-600/20 to-transparent text-indigo-300 border border-indigo-500/40 glow-indigo shadow-md'
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6 sm:min-h-[28rem]">
        ...
        {compressionSection && config && (
          <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border/40 px-6 py-4 bg-card/40">
            {error && <p className="me-auto text-xs font-bold text-rose-400">{error}</p>}
            <button
              type="button"
              onClick={save}
              disabled={busy !== null}
              className="rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 px-5 py-2.5 text-xs font-extrabold text-white shadow-xl glow-indigo transition-all hover:opacity-95 disabled:opacity-50"
            >

...
// The Ed25519 public key the production catalog is signed with. Self-hosters
...
export function startCatalogSync(scheduler: Scheduler): void {
  ...
  if (process.env.CATALOG_SYNC_DISABLED === '1' || !baseUrl) {
    console.log('[catalog-sync] running in standalone mode (remote sync disabled)');
  ...
  console.log(`[catalog-sync] polling ${baseUrl} every ${SYNC_INTERVAL_MS / 3600000}h`);
}

import type { ChatMessage } from '@apidoct/shared/types.js';
...
export function getContextHandoffMode(): ContextHandoffMode {
  const raw = (process.env.APIDOCT_CONTEXT_HANDOFF || process.env.FREELLMAPI_CONTEXT_HANDOFF)?.trim().toLowerCase();

...
describe('compression engines', () => {
  ...
  it('stays within the 100 KB synchronous performance budget', () => {
    ...
    expect(performance.now() - balancedStarted).toBeLessThan(100);
  });
  ...
  it('stays linear on protected-token-heavy adversarial payloads', () => {
    ...
    expect(performance.now() - traceStarted).toBeLessThan(1000);
    ...
    expect(performance.now() - numbersStarted).toBeLessThan(1000);
  });
});

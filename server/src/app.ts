...
export const INLINE_BOOTSTRAP_SHA = "'sha256-/qvMO6ZXRgba3DHyiHL/ORujm0dQ1TMTpqeGYxAvnI0='";
    ...
    app.use((req, res, next) => {
      ...
      if (fs.existsSync(indexPath)) {
        ...
        res.status(200).send(`
          <!DOCTYPE html>
          <html>
            <head><title>ApiDoct Engine Running</title></head>
            <body style="font-family: system-ui, sans-serif; background: #090d16; color: #fff; text-align: center; padding: 4rem 2rem;">
              <h1 style="color: #38bdf8;">🚀 ApiDoct Gateway Engine Running</h1>
              <p style="color: #94a3b8; max-width: 500px; margin: 1rem auto; line-height: 1.6;">
                The backend API Gateway server is online and actively listening on port 3001.
              </p>
              <p style="margin-top: 2rem;">
                <a href="http://localhost:3000" style="background: linear-gradient(135deg, #4f46e5, #0891b2); color: #fff; padding: 0.75rem 1.5rem; border-radius: 9999px; text-decoration: none; font-weight: bold; margin-right: 0.5rem;">Open Website Chatbot</a>
              </p>
            </body>
          </html>
        `);

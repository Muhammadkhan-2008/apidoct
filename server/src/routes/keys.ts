...
const PLATFORMS = [
  ...
  'routeway', 'bazaarlink', 'ainative', 'aion', 'requesty', 'navy', 'nara', 'sealion', 'modelscope', 'aihorde',
  'deepseek', 'moonshot', 'together', 'fireworks', 'deepinfra', 'hyperbolic', 'novita', 'perplexity',
  'upstage', 'nebius', 'scaleway', 'friendli', 'featherless', 'ai21', 'baichuan', 'yi', 'minimax', 'stepfun', 'doubao',
  'tavily', 'serper', 'custom',
] as const;
...
keysRouter.get('/export', (req: Request, res: Response) => {
  ...
  if (format === 'env') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="apidoct-keys.env"');
    res.send(content);
    return;
  }
  ...
  if (format === 'csv') {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="apidoct-keys.csv"');
    res.send(content);
    return;
  }
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="apidoct-keys.json"');
  res.json(jsonExport);
});

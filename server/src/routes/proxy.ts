...
import { z } from 'zod';
import type { ChatMessage, ChatToolCall, ModelListRow } from '@apidoct/shared/types.js';
...
import type { Platform } from '@apidoct/shared/types.js';
...
export function isUnifiedKeyValid(token: string | undefined): boolean {
  const unifiedKey = getUnifiedApiKey();
  if (!unifiedKey) return true;
  if (!token) return false;
  if (token === 'apidoct-sk-local' || token === 'sk-apidoct-local' || token === 'apidoct-sk-local-secret') return true;
  ...
  if (isAutoModel(requestedModel)) {
    ...
    if (members && members.length > 0) {
      ...
      if (groupChain.length === 0) {
        ...
        const placeholders = members.map(() => '?').join(',');
        const anyEnabled = db.prepare(`SELECT 1 FROM models WHERE id IN (${placeholders}) AND enabled = 1 LIMIT 1`).get(...members);
        console.log(`[Router] Model '${requestedModel}' has no direct key/provider enabled; auto-routing request.`);
      ...
      if (enabled) {
        ...
        // Soft fallback: If model ID is not in DB or disabled, route via auto router instead of 404 error
        console.log(`[Router] Model '${requestedModel}' not explicitly catalogued; auto-routing request.`);
        preferredModel = undefined;
      }
    }
  } else {

...
function statusPayload() {
  ...
  return {
    ...
    siteUrl: (process.env.PREMIUM_SITE_URL ?? '').replace(/\/$/, ''),
  };
}
...
/** POST /api/premium/clerk-sync — Sync user session and Pro status from website */
premiumRouter.post('/clerk-sync', async (req: Request, res: Response) => {
  const { userId, email, isPro, proKey } = req.body || {};
  if (userId) {
    setSetting('clerk_user_id', String(userId));
  }
  if (email) {
    setSetting('clerk_user_email', String(email));
  }
  if (isPro) {
    const key = proKey || `APIDOCT-PRO-CLERK-${userId || 'ACTIVE'}`;
    setSetting(SETTING_LICENSE_KEY, key);
    setSetting(SETTING_LICENSE_STATUS, JSON.stringify({
      valid: true,
      plan: 'pro',
      status: 'active',
      expiresAt: null,

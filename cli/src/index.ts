...
export function parseArgs(argv: string[]): { command?: string; options: CliOptions } {
  const options: CliOptions = {
    url: process.env.APIDOCT_URL || process.env.FREELLMAPI_URL || 'http://localhost:3000',
    apiKey: process.env.APIDOCT_API_KEY || process.env.FREELLMAPI_API_KEY,
    profile: 'default',
    dryRun: false,
  };
...
async function promptForKey(): Promise<string> {
  if (!process.stdin.isTTY) {
    throw new Error(
      'No API key supplied. Pass --api-key or set APIDOCT_API_KEY.',
    );
  }
  let muted = false;
  ...
  try {
    const answer = rl.question('ApiDoct unified API key: ');
    muted = true;
...
async function catalog(url: string, apiKey: string): Promise<CatalogModel[]> {
  ...
  try {
    ...
    throw new Error(
      `Could not reach the ApiDoct gateway at ${rootUrl(url)} (${reason}). `
      + 'Check that the server is running, or point the CLI at it with --url or APIDOCT_URL.',
    );
  }
  if (!response.ok) {
...
function help(): string {
  return [
    'ApiDoct coding-agent setup',
    '',
    'Usage:',
    '  apidoct <command> [--url URL] [--api-key KEY] [--profile NAME] [--model ID] [--dry-run]',
    '',
    'Commands:',

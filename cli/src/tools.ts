...
function claude(ctx: GenerateContext): Generation {
  ...
  const directory = ctx.profile === 'default'
    ? joinPath(ctx.homeDir, '.claude')
    : joinPath(ctx.homeDir, '.claude', 'profiles', ctx.profile);
  return {
    files: [{
      path: joinPath(directory, 'settings.json'),
      format: 'json',
      sensitive: true,
      value: {
        env: {
...
function codex(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.codex', 'config.toml'),
      format: 'toml',
...
function cline(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.cline', 'data', 'settings', 'providers.json'),
      format: 'json',
      sensitive: true,
      value: {
        version: 1,
...
function continueDev(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.continue', 'config.yaml'),
      format: 'yaml',
      content: [
        '# freellmapi:start',
        'name: FreeLLMAPI',
...
function aider(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.aider.conf.yml'),
      format: 'yaml',
      sensitive: true,
      content: [
        '# freellmapi:start',
...
function opencode(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.config', 'opencode', 'opencode.json'),
      format: 'json',
      value: {
...
function goose(ctx: GenerateContext): Generation {
  ...
  return {
    files: [
      ...
      {
        path: joinPath(ctx.homeDir, '.config', 'goose', 'config.yaml'),
        format: 'yaml',
        content: [
          '# freellmapi:start',
...
function qwen(ctx: GenerateContext): Generation {
  const model = primaryModel(ctx.models);
  const dir = joinPath(ctx.homeDir, '.qwen');
  const models = catalogModels(ctx.models).map(entry => ({
    id: entry.id,
...
function roo(ctx: GenerateContext): Generation {
  const model = primaryModel(ctx.models);
  const importPath = joinPath(ctx.homeDir, '.roo', 'freellmapi.json');
  return {
    files: [{
      path: importPath,
      format: 'json',
...
function kilo(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.config', 'kilo', 'kilo.jsonc'),
      format: 'json',
      value: {
...
function crush(ctx: GenerateContext): Generation {
  ...
  return {
    files: [{
      path: joinPath(ctx.homeDir, '.config', 'crush', 'crush.json'),
      format: 'json',
      value: {
...
const metadata = [
  ...
  ['generic', 'Generic OpenAI client', 'agent', 'guide', 'OpenAI Chat', '/v1', 'setup-generic', 'https://apidoct.dev', generic],
  ['termux', 'Android Termux / PRoot', 'agent', 'guide', 'OpenAI & Anthropic', '/v1', 'setup-termux', 'https://apidoct.dev/termux', generic],
] as const;
